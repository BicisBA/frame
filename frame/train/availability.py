...
class AvailabilityModelTrainer:
    ...
    def train_station(self, station_id, data_set) -> None:
        station_pipeline = make_pipeline(
            ColumnTransformer(
                [
                    ("ohe", OneHotEncoder(sparse=False), self.ohe_slice),
                    ("ss", StandardScaler(), self.ss_slice),
                ]
            ),
        ...
        self.stations_metrics[station_id] = confusion_matrix(
            y_test, station_pipeline.predict(X_test), labels=[1, 0], normalize="true"
        )
